...
set "nceline=echo: &echo ==== 出错 ==== &echo:"
set "eline=echo: &call :_color %Red% "==== 出错 ====" &echo:"
...
%nceline%
echo 检测到不支持的操作系统版本
echo 项目仅支持 Windows 7/8/8.1/10/11 及其等效服务器
goto MASend
)
...
%nceline%
echo 脚本从临时文件夹启动
echo 您很可能直接从压缩包中运行了脚本
echo:
echo 解压文件并从解压后的文件夹中启动脚本
goto MASend
)
)
...
echo:
echo:
call :_color2 %_Yellow% "         Windows/Office 三合一万能激活工具[AIO 1.8]   " %_Green% "汉化: Ma5老师"
echo:
echo:       ______________________________________________________________
echo:
echo:                 激活方法:
echo:
echo:             [1] 数字(HWID)    ^|  Windows           ^|   永久
echo:             [2] KMS38         ^|  Windows           ^|   至2038年
echo:             [3] 在线KMS       ^|  Windows/Office    ^|   180天
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "          " %_Green% "根据菜单选项单击数字键 [1,2,3,4,5,6,7,0] :"
choice /C:12345670 /N
set _erl=%errorlevel%
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "根据菜单选项单击数字键 [1,2,3,4,0] :"
choice /C:12340 /N
set _erl=%errorlevel%
...
echo:             [3] 在线KMS
echo:             
echo:             [4] 硬件HWID  ^(Windows^) ^+ 在线 KMS ^(Office^)
echo:             [5] KMS38 ^(Windows^) ^+ 在线 KMS ^(Office^)
echo:             __________________________________________________      
echo:                                                                   
echo:             [0] 返回
echo:       ______________________________________________________________
echo:  
call :_color2 %_White% "           " %_Green% "单击选项中的数字键:"
choice /C:123450 /N
set _erl=%errorlevel%
...
if defined _error (
%eline%
echo 未能在桌面上提取 $OEM$ 文件夹
) else (
echo:
call :_color %Magenta% "%_oem%"
call :_color %Green% $OEM$ 文件夹已经成功的被创建到桌面上
)
echo "%_oem%" | find /i "KMS38" 1>nul && (
echo:
echo 至KMS38激活服务器 Cor/Acor editions ^(No GUI Versions^),
echo 查检此页 https://massgrave.dev/oem-folder
)
...
set "nceline=echo: &echo ==== 出错 ==== &echo:"
set "eline=echo: &call :dk_color %Red% "==== 出错 ====" &echo:"
...
%eline%
echo HWID数字式激活不支持Windows服务器版
echo 使用KMS38或在线KMS激活选项
goto dk_done
)
...
call :dk_color %Red% "启动 [sppsvc] 服务失败, 余下的处理可能需要很长时间..."
echo:
)
...
echo:
call :dk_color2 %_White% "     " %Green% "检测到: %winos% 已经永久激活！"
...
echo 检测系统信息              [%winos% ^| %winbuild% ^| %arch%]
...
set error=1
call :dk_color %Red% "正检测网络连接           [没有连接]"
)
...
echo Enabling Windows Script Host            [成功]
)
...
call :dk_color %Red% "正检查HWID或选版本     [%altedition% 激活 ID 没找到]"
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-*EvalEdition~*.mum" (
call :dk_color %Magenta% "发现Windows评估版，安装完整版 %winos%"
call :dk_color %Magenta% "从以下网址下载 https://massgrave.dev/genuine-installation-media.html"
)
)
...
call :dk_color %Magenta% "[%altedition%] 版本产品密钥将用于启用硬件ID(HWID)激活。"
echo:
)
...
if %errorcode% EQU 0 (
call :dk_refresh
echo Installing Generic Product Key          [%key%] [成功]
) else (
call :dk_color %Red% "Installing Generic Product Key          [%key%] [失败] %errorcode%"
if not defined error (
call :dk_color %Magenta% "在主菜单下, 选择问题排查并运行修复授权选项。"
if defined actidnotfound call :dk_color %Red% "此秘钥的激活ID没有找到，请确保你使用的是最新的更新版激活工具！"
set showfix=1
)
set error=1
)
...
if !errorlevel! EQU 0 (
echo Changing Windows Region To USA          [成功]
) else (
call :dk_color %Red% "Changing Windows Region To USA          [失败]"
)
)
...
call :dk_color %Red% "Generating GenuineTicket.xml            [失败]"
echo [%encoded%]
if exist "%tdir%\Genuine*" del /f /q "%tdir%\Genuine*" %nul%
goto :dl_final
) else (
echo 生成GenuineTicket.xml    [成功]
)
...
set error=1
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "Installing GenuineTicket.xml            [ClipSVC 服务重启失败，等待...]"
)
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "Installing GenuineTicket.xml            [clipup -v -o 操作失败]"
)

if exist "%ProgramData%\Microsoft\Windows\ClipSVC\Install\Migration\*.xml" (
set error=1
set rebuildinfo=1
call :dk_color %Red% "Checking Ticket Migration               [失败]"
)
...
if defined _perm (
echo:
call :dk_color %Green% "%winos% 使用数字授权永久激活。"
goto :dl_final
)
...
set error=1
echo:
call :dk_color %Red% "正检查授权服务             [连接失败]"
call :dk_color2 %Magenta% "查检此页 for help" %_Yellow% " https://massgrave.dev/licensing-servers-issue"
)
)
...
echo Deleting a Registry                     [成功] [!_ident!]
)
...
call :dk_color %Magenta% "在保存时, 此产品不支持HWID激活。"
call :dk_color %Magenta% "使用KMS38激活选项"
) else (
if not defined error call :dk_color %Magenta% "在主菜单下, 选择问题排查并运行修复授权选项。"
call :dk_color2 %Magenta% "查检此页 for help" %_Yellow% " https://massgrave.dev/troubleshoot"
)
)
...
if %osSKU%==175 call :dk_color %Red% "%winos% 在 non-azure 平台不支持激活。"
...
if %_wmic% EQU 0 %psc% "(([WMISEARCHER]'SELECT Name FROM SoftwareLicensingProduct WHERE LicenseStatus=1 AND GracePeriodRemaining=0 AND PartialProductKey IS NOT NULL').Get()).Name | %% {echo ('名称='+$_)}" 2>nul | findstr /i "Windows" 1>nul && set _perm=1||set _perm=
exit /b
...
if defined serv_e (
set error=1
call :dk_color %Red% "开启服务                       [失败] [%serv_e%]"
echo %serv_e% | findstr /i "ClipSVC-1058 sppsvc-1058" %nul% && (
call :dk_color %Magenta% "重启系统修复没有开启的服务错误 1058."
)
)
...
set error=1
call :dk_color2 %Red% "Checking Boot Mode                      " %Magenta% "[系统运行在了安装模式，请以正常模式运行。]"
)


reg query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Setup\State" 2>nul | find /i "IMAGE_STATE_COMPLETE" 1>nul || (
set error=1
call :dk_color2 %Red% "Checking Audit Mode                     " %Magenta% "[系统运行在了编辑模式，请以正常模式运行。]"
)


reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinPE" /v InstRoot %nul% && (
set error=1
call :dk_color2 %Red% "Checking WinPE                          " %Magenta% "[系统运行了在了WinPE模式，请以正常模式运行。]"
)


for %%# in (wmic.exe) do @if "%%~$PATH:#"=="" (
call :dk_color %Gray% "检查 WMIC.exe     [未找到]"
)


%psc% $ExecutionContext.SessionState.LanguageMode 2>nul | find /i "Full" 1>nul || (
set error=1
call :dk_color %Red% "正在检测 Powershell                     [没有反应]"
)
...
set error=1
set wmifailed=1
call :dk_color %Red% "Checking WMI                            [没有反应] %_wmic%"
call :dk_color %Magenta% "在主菜单, 选择问题排查并运行修复 WMI 选项。"
)


if not "%regSKU%"=="%wmiSKU%" (
call :dk_color %Red% "Checking WMI/REG SKU                    [发现差别 - WMI:%wmiSKU% Reg:%regSKU%]"
)
...
if %error_code% NEQ 0 (
call :dk_color %Red% "正在检测 DISM                           [没有反应] %error_code%"
)


if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-*EvalEdition~*.mum" (
set error=1
call :dk_color %Red% "Checking Eval Packages                  [评估版windows没有安装评估许可证]"
)
...
if %error_code% NEQ 0 (
set error=1
call :dk_color %Red% "正在检测 slmgr /dlv                     [没有反应] %error_code%"
)


reg query "HKU\S-1-5-20\Software\Microsoft\Windows NT\CurrentVersion\SoftwareProtectionPlatform\PersistedTSReArmed" %nul% && (
set error=1
call :dk_color2 %Red% "Checking Rearm                          " %Magenta% "[需要重启系统]"
)


reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ClipSVC\Volatile\PersistedSystemState" %nul% && (
set error=1
call :dk_color2 %Red% "Checking ClipSVC                        " %Magenta% "[需要重启系统]"
)
...
call :dk_color %Red% "正在检测 SkipRearm                      [默认0值没有找到，更改为0]"
...
call :dk_color %Red% "Checking SPP Registry Key               [发现不正确模块]"
set error=1
)
...
if not defined applist (
set error=1
call :dk_color %Red% "Checking Activation IDs                 [没有找到]"
)
)
...
set error=1
call :dk_color %Red% "Checking SPP Token Folder               [没有找到] [%tokenstore%\]"
)


if exist "%tokenstore%\" if not exist "%tokenstore%\tokens.dat" (
set error=1
call :dk_color %Red% "Checking SPP tokens.dat                 [没有找到] [%tokenstore%\]"
)


if not exist %ProgramData%\Microsoft\Windows\ClipSVC\tokens.dat (
set error=1
call :dk_color %Red% "Checking ClipSVC tokens.dat             [没有找到]"
)


if not exist %SystemRoot%\system32\sppsvc.exe (
set error=1
call :dk_color %Red% "Checking sppsvc.exe File                [没有找到]"
)
...
if defined wpaerror call :dk_color %Red% "Checking WPA Registry Keys              [发现错误] [Registry Count - !count!]"
)
...
if defined permerror call :dk_color %Red% "Checking SPP Permissions                [发现错误]"

set showfix=1
call :dk_color %Magenta% "在主菜单下, 选择问题排查并运行修复授权选项。"
if not defined permerror call :dk_color %Magenta% "如果激活仍然出错则运行修复 WPA 注册表选项。"
)
...
%eline%
echo 检测到不支持的操作系统版本
echo KMS38 激活用于 Windows 10/11/Server, build 14393 及后续更新版。
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "根据菜单选项单击数字键 [1,2,0]"
choice /C:120 /N
set _el=!errorlevel!
if !_el!==3  exit /b
...
echo 服务器评估无法激活。将其转换为完整的服务器操作系统。
echo:
echo 在主菜单, 选择扩展，并且使用 '更改版本' 选项
) else (
echo 评估版不能激活
echo 你需要安装 %winos% 完整版
echo:
...
%eline%
echo clipup.exe 不存在于服务 Cor/Acor [No GUI] 版本
echo 需要 KMS38 激活
echo 关于激活请检测下面网页
...
call :dk_color %Red% "检查 KMS38 备用版本   [%altedition% 激活ID没有找到]"
)
...
if %_wmic% EQU 0 for /f "tokens=2 delims==" %%# in ('%psc% "(([WMISEARCHER]'SELECT PartialProductKey FROM SoftwareLicensingProduct WHERE ApplicationID=''55c92734-d682-4d71-983e-d6ec3f16059f'' AND PartialProductKey IS NOT NULL').Get()).PartialProductKey | %% {echo ('部分产品秘钥='+$_)}" 2^>nul') do set "_partial=%%#"
call echo 检查已安装的产品密钥          [Partial Key - %%_partial%%] [Volume:GVLK]
)
...
if !error_code! EQU 0 (
call :dk_refresh
echo 安装 KMS 客户端设置密钥 [%key%] [成功]
) else (
call :dk_color %Red% "正在安装 KMS 客户端安装密钥         [%key%] [失败] !error_code!"
if not defined error (
call :dk_color %Magenta% "在主菜单下, 选择问题排查并运行修复授权选项。"
set showfix=1
)
...
if not defined app (
call :dk_color %Red% "Checking Installed GVLK Activation ID   [没有找到] 中止..."
goto :dk_done
)
...
echo 添加特定KMS主机 [LocalHost 127.0.0.2] [成功]
) else (
call :dk_color %Red% "添加指定 KMS 主机               [LocalHost 127.0.0.2] [失败]"
)
...
call :dk_color %Red% "Copying clipup.exe File to              [%systemroot%\System32\] [失败] 中止..."
goto :k_final
)
)
...
echo Stopping sppsvc Service                 [成功]
) || (
call :dk_color %Red% "Stopping sppsvc Service                 [失败]"
)
...
if %errorlevel%==0 (
echo 应用 SKU-ID Rearm                   [成功]
) else (
call :dk_color %Red% "应用 SKU-ID Rearm                   [失败]"
)
call :dk_refresh
...
%nul% reg delete "HKLM\%specific_kms%" /f
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Magenta% "Protect KMS38 By KMS                    [成功] [Locked A Registry Key]"
) || (
call :dk_color %Red% "Protect KMS38 By KMS                    [锁定注册秘钥失败]"
)
)
...
if defined a_cor (
if exist "%_clipup%" (
call :dk_color %Red% "Deleting copied clipup.exe file         [失败]"
) else (
...
call :dk_color %Red% "%winos% 在 non-azure 平台不支持激活。"
)
...
echo:
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Red% “删除特定KMS主机 [失败]”
) || (
echo 删除特定KMS主机 [成功]
)
...
call :dk_color %Green% "%winos% 已经激活直到 !xpr!"
exit /b
...
if %_wmic% EQU 0 for /f "tokens=2 delims==" %%# in ('%psc% "(([WMISEARCHER]'SELECT GracePeriodRemaining FROM SoftwareLicensingProduct WHERE ApplicationID=''55c92734-d682-4d71-983e-d6ec3f16059f'' AND Description like ''%%KMSCLIENT%%'' AND PartialProductKey IS NOT NULL').Get()).GracePeriodRemaining | %% {echo ('剩余宽限期='+$_)}" 2^>nul') do set "gpr=%%#"
...
if %_wmic% EQU 0 for /f "tokens=2 delims==" %%# in ('%psc% "(([WMISEARCHER]'SELECT ProductKeyChannel FROM SoftwareLicensingProduct WHERE ApplicationID=''55c92734-d682-4d71-983e-d6ec3f16059f'' AND PartialProductKey IS NOT NULL').Get()).ProductKeyChannel | %% {echo ('产品秘钥通道='+$_)}" 2^>nul') do set "_channel=%%#"
exit /b
...
if defined _oldtsk       call :_color2 %_White% "              [4] 安装自动续订      " %_Red% "[老式安装]"
...
call :_color2 %_White% "              [6] 开启调试模式        " %_Red% "[是]"
)
...
if %_Debug% EQU 0 (
  ...
  echo.
  echo 在调试模式下运行…
  if not defined _args (echo 完成后窗口将会关闭) else (echo 请稍等...)
  echo.
  echo 将调试日志写入：
...
if defined _unattended (
echo.
call :_color %_Red% "互联网不能，无论如何都进行处理"
) else (
%eline%
echo 互联网未连接。
echo:
...
if %ActOffice% EQU 0 (echo.&echo Office激活被关掉...)
...
if %WinVL% EQU 0 (
if %ActWindows% EQU 0 (
  echo.&echo Windows 激活已经关闭..
  ) else (
  if %SSppHook% EQU 0 (
    echo.&echo %_winos% %nKMS%
    if defined _eval echo %nEval%
    ) else (
    echo.&echo 检查 KMS 用于Windows激活ID^(s^) 失败。 &call :CheckWS
    exit /b
    )
  )
)
...
if %W1nd0ws% EQU 1 if %ActWindows% EQU 0 (echo.&echo Windows 激活已经关闭..)
...
if %winbuild% LSS 9200 (if %loc_off14% EQU 0 (echo.&echo 未检测到 Office %aword% Product...&exit /b))
)
...
if %WinPerm% EQU 1 (
for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 检测: %%x
echo 产品已经永久激活
exit /b
)
call :insKey
...
if %OsppHook% EQU 0 (echo.&echo 没被安装的 Office %aword% 被检测到...&exit /b)
if %winbuild% GEQ 9200 if %loc_off14% EQU 0 (echo.&echo 没被安装的 Office %aword% 被检测到...&exit /b)
set err_offsvc=0
net start osppsvc /y %_Nul3% || (
sc start osppsvc %_Nul3%
if !errorlevel! EQU 1053 set err_offsvc=1
)
if %err_offsvc% EQU 1 (echo.&echo错误: osppsvc 服务未运行...&exit /b)
...
set "_key="
set "_qr=%_zz7% %spp% %_zz2% %_zz5%ID='%app%'%_zz6% %_zz3% Name %_zz8%"
if %ESU_ADD% EQU 0 for /f "tokens=2 delims==" %%x in ('%_qr%') do echo 安装秘钥: %%x
if %ESU_ADD% EQU 1 for /f "tokens=2 delims==f" %%x in ('%_qr%') do echo 安装秘钥: %%x
set ESU_ADD=0
call :keys %app%
...
if %OSType% EQU Win7 echo 由于 OEM BIOS 不符合要求，无法在此计算机上激活 Windows 7。
echo 详情请参阅自述文件。
exit /b
)
...
call :_color %_Green% "产品已经成功激活"
echo 剩余时间: %gpr2% days ^(%gpr% minutes^)
set /a act_attempt=0
exit /b
)
...
if "%_ProductIds%"=="" (
if %_Office15% EQU 0 (echo 错误：未检测到 Office C2R 产品 ID&goto :%_fC2R%) else (goto :Reg15istry)
)
...
if "%_Product15Ids%"=="" (
if %_Office16% EQU 0 (echo 错误：未检测到 Office 2013 C2R 产品 ID&goto :%_fC2R%) else (goto :CheckC2R)
)
...
echo O365专业增强 2016 套装 ^<-^> Mondo 2016 许可证
...
set _O365ProPlus=1
echo O365Business 2016 套装 ^<-^> Mondo 2016 许可证
...
set _O365ProPlus=1
echo O365SmallBusPrem 2016 套装 ^<-^> Mondo 2016 许可证
...
set _O365ProPlus=1
echo O365HomePrem 2016 套装 ^<-^> Mondo 2016 许可证
...
set _O365ProPlus=1
echo O365EduCloud 2016 套装 ^<-^> Mondo 2016 许可证
...
if !_O365ProPlus! EQU 0 if !_ProPlus2021! EQU 0 if !_ProPlus2019! EQU 0 if !_ProPlus! EQU 0 if !_Professional2021! EQU 0 if !_Professional2019! EQU 0 if !_Professional! EQU 0 if !_Standard2021! EQU 0 if !_Standard2019! EQU 0 if !_Standard! EQU 0 if !_%%a2021! EQU 0 (
  set _Standard2019=1
  echo %%a 2019 Suite -^> Standard%_ons% 许可证
  call :InsLic Standard%_tag%
  )
)
...
echo O365专业增强 2013 套装 ^<-^> Mondo 2013 许可证
...
set _O365ProPlus=1
echo O365SmallBusPrem 2013 套装 ^<-^> Mondo 2013 许可证
...
set _O365ProPlus=1
echo O365HomePrem 2013 套装 ^<-^> Mondo 2013 许可证
...
set _O365ProPlus=1
echo O365Business 2013 套装 ^<-^> Mondo 2013 许可证
...
echo SharePoint Designer 2013 应用程序 -^> Mondo 2013 许可证
call :Ins15Lic Mondo
goto :GVLKC2R
)
...
if not [%Act_OK%]==[1] (
echo.
echo 关于问题, 检查 https://massgrave.dev/troubleshoot
)
...
call :_color %_Red% "网络没有连接。"
exit /b
)

if [%ERRORCODE%]==[-1073418124] (
echo 检查端口1688 的连接, 可能需要一段时间...
echo.
...
call :_color %Red% "端口1688 在网络上受阻。"
echo.
echo Reason:   或许受限互联网[办公/校园]已经连接。
echo           或防火墙阻止了此连接。
echo.
echo 解决方法: 用另一个网络上网或使用离线 KMS
echo           https://github.com/abbodi1406/KMS_VL_ALL_AIO
) else (
echo 端口 1688 连接测试通过。
echo.
echo 确保系统文件没有被防火墙阻拦。
echo 如果问题仍然存在，试试离线 KMS
echo https://github.com/abbodi1406/KMS_VL_ALL_AIO
)
echo.
)

echo 在这种情况下，回显 KMS 服务器不是问题。
exit /b
...
echo %_dest%\Info.txt
echo.
(if defined ActTask (echo 计划任务被创建:) else (echo 计划任务被创建:))
echo \激活-更新 [每周]
if defined ActTask (echo \Activation-Run_Once)
echo __________________________________________________________________________________________
echo.
echo 信息：
echo 如果找到 Internet 连接，将每周更新激活。
echo 仅续订已经安装的 KMS 许可证，它不会将任何授权转为 KMS。
echo __________________________________________________________________________________________
echo.
if defined ActTask (
...
echo.
call :_color %Gray% "确何你至少运行过一次激活选项。"
echo __________________________________________________________________________________________
)
...
(
echo   此脚本会使用在线 KMS 为你续订Windows/Office KMS 许可证。
echo:
echo   如果创建了续订/激活计划任务，则将存在以下内容，
echo:
echo   - 计划任务
echo     激活-续订[续订/每周]
echo     Activation-Run_Once [激活任务 - 激活后自行删除]
...
echo:
echo   在线 KMS 激活脚本是“Microsoft 激活脚本”[MAS] 项目的一部分
echo:   
echo   主页: massgrave.dev
...
if %_wmic% EQU 0 for /f "tokens=2 delims==" %%x in ('%psc% "(([WMISEARCHER]'SELECT Name FROM !_path! WHERE ID=''!_actid!''').Get()).Name | %% {echo ('名称='+$_)}" 2^>nul') do call echo Activating: %%x
exit /b
...
echo 产品激活失败：0xC004F035
if [%1]==[act_win] if %winbuild% LSS 9200 echo 由于 OEM BIOS 不符合要求，无法在此计算机上激活 Windows 7
exit /b
)
...
if %_wmic% EQU 0 for /f "tokens=2 delims==" %%# in ('%psc% "(([WMISEARCHER]'SELECT GracePeriodRemaining FROM !_path! where ID=''!_actid!''').Get()).GracePeriodRemaining | %% {echo ('剩余宽限期='+$_)}" 2^>nul') do call set "gpr=%%#"
exit /b
...
cscript //nologo slmgr.vbs /dli || (echo 执行出错 slmgr.vbs&del /f /q slmgr.vbs&popd&goto :casVend)
...
if %GracePeriodRemaining% EQU 0 (
  if %winID% EQU 1 (set "ExpireMsg=此机子已经永久激活") else (set "ExpireMsg=此产品已经永久激活")
  ) else (
  set "LicenseMsg=%_mTag% 激活到期 %GracePeriodRemaining% 分钟(s) (%_gpr% 天(s))"
  if defined _xpr set "ExpireMsg=%_mTag% 激活到期时间: %_xpr%"
  )
)
...
if defined ProductKeyChannel echo 产品秘钥通道: %ProductKeyChannel%
echo 部分产品秘钥: %PartialProductKey%
echo 授权状态: %License%
...
echo 最新激活信息:
echo 密钥管理服务客户信息
echo.    客户端机器 ID (CMID): %ClientMachineID%
echo.    %KmsDns%
echo.    %KmsReg%
if defined DiscoveredKeyManagementServiceMachineIpAddress echo.    KMS机器IP地址： %DiscoveredKeyManagementServiceMachineIpAddress%
echo.    KMS机器扩展PID: %KeyManagementServiceProductKeyID%
...
%eline%
echo 检测到不支持的操作系统版本。
echo 此命令仅支持 Windows 8/8.1/10/11 及其等效服务器。
goto :at_back
)
...
call :_color2 %_White% "     " %Red% "检测互联网连接  [没有连接]"
)
...
echo:
call :_color %Gray% "CBS 和 DISM 日志被复制到桌面上的 ACT Logs 文件夹中"
goto :at_back
...
echo:
echo      如果 SFC 无法修复某些问题，则再次运行该命令以查看它是否能够
echo      到下一次 有时可能需要运行 sfc /scannow 命令 3 次
echo      每次后重新启动电脑，以完全修复它能够修复的所有内容
echo:   
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] 返回 : "
...
echo:
call :_color %Gray% "CBS 日志被复制到桌面 AT_Logs 文件夹下。"
goto :at_back
...
echo:
echo       - 此选项将会，
echo            - 停止 Windows 及 Office激活，你可能需要重新激活
echo            - 清除 ClipSVC, Office vNext, SPP and OSPP 许可证
echo            - 修复令牌文件夹和注册表的 SPP 权限
echo            - 触发 Office 的修复选项
echo:
call :_color2 %_White% "      - " %Red% "请在必要时应用此功能！"
echo:
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] 返回 : "
...
echo %line%
echo:
call :_color %Magenta% "正重建 ClipSVC 许可证"
echo:

if %winbuild% LSS 10240 (
echo ClipSVC 许可证重建仅支持 Win 10/11 and Server。
echo 正在跳过...
goto :cleanvnext
)
...
echo:
echo 正删除不稳定 ^& 被保护的注册表项...
echo [%RegKey%]
reg query "%RegKey%" %nul% && (
call :_color %Red% "[失败]"
echo 重启系统，会自动删除此注册表项。
) || (
echo [成功]
)
...
echo:
echo 重启 [wlidsvc LicenseManager] 服务...
...
echo 没有找到 - !ProgramData!\Microsoft\Office\Licenses\
)
...
echo 没有找到 - !_Local!\Microsoft\Office\Licenses\
)
...
echo %line%
echo:
call :_color %Magenta% "重建 SPP 许可令牌"
echo:
...
echo:
echo 检查 WPA 注册表项...
call :_color %Red% "[查找出错] [Registry Count - !count!]"
)
)
...
echo %line%
echo:
call :_color %Magenta% "重建 OSPP 许可令牌"
echo:

sc qc osppsvc %nul% || (
echo OSPP based Office 没有安装
echo 跳过得建 OSPP 令牌...
goto :repairoffice
)
...
echo:
echo 启动 osppsvc 服务生成 tokens.dat
...
echo %line%
echo:
call :_color %Magenta% "修复Office许可证"
echo:
...
if /i "%arch%"=="ARM64" (
echo:
echo 发现ARM64 Windows
echo 您需要在Windows设置中使用修复Office选项.
echo:
...
echo:
call :_color %_Yellow% "将会弹出个窗口，在窗口中你需要选择 [快速] 修复选项..."
...
if defined uwp16 (
echo:
echo 注意：跳过 Office 16.0 UWP 的修复 
...
if defined c2r14 (
echo:
echo 注意：跳过 Office 14.0 C2R 的修复
echo       您需要在 Windows 设置中使用修复选项
echo ________________________________________________________________
echo:
start appwiz.cpl
)
...
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
%eline%
echo WMI rebuild is not recommended on Windows Server. 中止...
goto :at_back
)

for %%# in (wmic.exe) do @if "%%~$PATH:#"=="" (
%eline%
echo wmic.exe file is not found in the system. 中止...
goto :at_back
)
...
call :_color %Red% "[没有反应]"

echo:
sc query Winmgmt %nul% || (
%eline%
echo Winmgmt service is not installed. 中止...
goto :at_back
)
...
call :_color %Red% "[失败]"
echo:
call :_color %Magenta% "建议选择 [重启] 选项然后再次使用修复 WMI 选项。"
echo %line%
echo:
...
echo:
echo 正检查 WMI
call :_color %Green% "[正工作中]"
goto :at_back
)

echo:
echo 正注册 .dll's 及 Compiling .mof's, .mfl's
...
call :_color %Red% "[没有反应]"
echo:
echo 运行 [Dism 恢复健康] and [SFC 现在扫描] 选项并且确认没有错误。
) else (
call :_color %Green% "[正工作中]"
)
...
%line%
echo:
echo 安装 [%winos% ^| SKU:%osSKU% ^| %winbuild%] %channel% 秘钥
echo [%key%]
%line%
echo:
if not "%regSKU%"=="%wmiSKU%" (
echo 注意：在 SKU 值中发现差异 - WMI:%wmiSKU% Reg:%regSKU%
echo:
)
call :dk_color %_Green% "单击 [1] 继续或单击 [0] %_exitmsg%"
choice /C:01 /N
...
echo 正在安装 [%key%]
echo:
call :dk_color %Red% "[没有成功] %error_code%"
if defined actidnotfound call :dk_color %Red% "此秘钥的激活ID没有找到"
echo 查检此页 for help https://massgrave.dev/troubleshoot
)
%line%
...
if not defined applist (
%eline%
echo Activation IDs not found. 中止...
echo:
echo 查检此页 for help. https://massgrave.dev/troubleshoot
goto ced_done
)
...
%line%
echo:
call :dk_color %Gray% "你可将版本[%osedition%] [%winbuild%]更改为下列中的一种"
%line%
echo:
...
call :dk_color %_Green% "输入选项数字, 然后"回车":"
set /p inpt=
...
if not [%%#]==[] (
%eline%
echo 在将版本从 [%osedition%] 更改到 [%targetedition%] ，需要断网
echo 断开互联网，然后按任意键...
pause >nul
goto ced_loop
)
...
if %_changepk%==1 (
call :dk_color %Magenta% "记录-"
echo:
echo  - 如果在更新窗口出现了错误，你可以安全忽略。
echo    在此情况下，你需要手动重启系统。
echo:
echo  - 在继续工作前，请先保存你的工作，系统将会自动重启。
echo  - 系统重启后，你可以连网的。
echo:
echo  - 当版本更改后，你需要使用一次 HWID 激活选项。
echo:
...
call :dk_color %Red% "[没有成功] [Error Code: 0x!=ExitCode!]"
)
)
...
echo 将当前版本 [%osedition%] 更改为 [%targetedition%]
echo:
call :dk_color %Magenta% "重要 - 在继续之前保存您的工作，系统将自动重启"
if %winbuild% GEQ 17034 if %targetedition%==Professional echo           - 企业版秘钥将被安装并替代专业版本秘钥，以后你也可快速的改回专业版。 
echo:
